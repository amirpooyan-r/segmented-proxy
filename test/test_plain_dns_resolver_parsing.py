...
def _build_response(qtype: int, ttl: int, rdata: bytes) -> bytes:
    txid = 0x1A2B
    flags = 0x8180
...
def test_parse_a_record() -> None:
    ...
    addrs, ttl = PlainDnsResolver._parse_response(data, A_RECORD)
...
def test_parse_aaaa_record() -> None:
    ...
    addrs, ttl = PlainDnsResolver._parse_response(data, AAAA_RECORD)
